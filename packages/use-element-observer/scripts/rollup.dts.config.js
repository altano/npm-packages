...
const bundledDestination = "dist/oxc-bundled";
if (bundledDestination && existsSync(bundledDestination)) {
  rmSync(bundledDestination, { recursive: true });
}

if (!existsSync("temp/src")) {
  console.warn(
...
const src = readdirSync("temp/src");
const targets = process.env.TARGETS ? process.env.TARGETS.split(",") : null;
const targetSrc = targets ? src.filter((pkg) => targets.includes(pkg)) : src;

export default targetSrc.map(
  /** @returns {import('rollup').RollupOptions} */
  (pkg) => {
    return {
      input: `./temp/src/${pkg}`,
      output: {
        file: `dist/dts/oxc-rollup/index.d.ts`,
        format: "es",
      },
...
function patchTypes(pkg) {
  return {
    ...
    renderChunk(code, chunk) {
      ...
      const additionalTypeDir = `src/${pkg}/types`;
...
function copyMts() {
  return {
    ...
    writeBundle(_, bundle) {
      ...
      writeFileSync("src/vue/dist/vue.d.mts", bundle["vue.d.ts"].code);
    },
  };
}
