...
describe("with-minification", () => {
  let fixture: Fixture;

  beforeAll(async () => {
    fixture = await loadFixture({
      root: "../fixtures/with-minification/",
      output: "static",
      integrations: [
        prettierResponse({
  ...
  it("index.astro page should be formatted", async () => {
    ...
    //   - In the CSS, the comments and whitespace are stripped
    //   - The HTML is the same, since compressHTML only performs transforms that Prettier completely reverses
    //   - In the JS, the symbol names are mangled (e.g. `doSomething` => `n`)
    //
