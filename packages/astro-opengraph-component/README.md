...
```sh
# Using NPM
npx astro add @altano/astro-opengraph-component
# Using Yarn
yarn astro add @altano/astro-opengraph-component
# Using PNPM
pnpm astro add @altano/astro-opengraph-component
```
...
Configure the integration in your Astro config. You must at least provide some fonts to use (as there are no defaults), e.g.:
...
In order serve the component at a given route, we need a js/ts [static file endpoint](https://docs.astro.build/en/guides/endpoints/#static-file-endpoints). You can use a static path or dynamic path. You can name the endpoint filename whatever you want as long as it has the extension `.png.ts`. For example:

`src/pages/opengraph.png.ts`:

```ts
import opengraphTemplate from "../components/opengraph.astro";
import { makeOpengraphEndpoint } from "@altano/astro-opengraph-component";
...
```astro
---
import OpenGraphMeta from "@altano/astro-opengraph-component/components/meta.astro";
---
...
<html>
  ...
  <body>
    <article>Golfers and Conservatives – Why Control Was Right</article>
  </body>
</html>
```
...
You can use the `OpenGraphMeta` default props but you probably want to at least provide a `title`, `description`, and, if you used something other than `opengraph.png.ts` as your endpoint, you'll want to specify the path to the image endpoint with the `image` prop.
...
Once you repeat the steps above for every Open Graph card you want to render, you should have something like this:
...
Or, if you decided to colocate the .astro templates in the pages directory, you'd have something like this:
...
See https://github.com/altano/npm-packages/tree/main/examples/astro-opengraph-component for a complete example.
...
You can find an example here: [tests/fixtures/catch-all](https://github.com/altano/npm-packages/packages/astro-opengraph-component/tests/fixtures/catch-all)
...
While in `astro dev` mode, you can use the [`astro-meta-tags` integration](https://github.com/patrick91/astro-meta-tags) to easily preview your opengraph image/tags for a given page to make sure they are rendering correctly.
